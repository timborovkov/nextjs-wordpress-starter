...
/**
 * Get dictionary translations by group for a specific language
 * @param group - Group name (e.g., 'navigation', 'buttons')
 * @param lang - Language code (e.g., 'en', 'fi'). If not provided, uses current language
 * @returns Promise<DictionaryGroupResponse>
 */
export async function getDictionaryTranslationsByGroup(
  group: string,
  lang?: string
): Promise<DictionaryGroupResponse> {
  ...
  return wordpressFetch<DictionaryGroupResponse>(
    "/wp-json/wp/v2/dictionary-translations/group",
    query
  );
}
 ...
 * @param key - Translation key (e.g., 'nav_home', 'btn_submit')
 * @param lang - Language code (e.g., 'en', 'fi'). If not provided, uses current language
 * @param default - Default value if translation not found
 * @returns Promise<string>
 */
export async function getDictionaryTranslation(
  key: string,
  lang?: string,
  default_value: string = key
): Promise<string> {
  try {
    const response = await getDictionaryTranslations(lang);
    return response.translations[key] || default_value;
  } catch (error) {
    console.warn(
      `Failed to fetch dictionary translation for key '${key}':`,
      error
    );
...
/**
 * Get multiple dictionary translations by keys and language
 * @param keys - Array of translation keys
 * @param lang - Language code (e.g., 'en', 'fi'). If not provided, uses current language
 * @returns Promise<DictionaryTranslation>
 */
export async function getDictionaryTranslationsByKeys(
  keys: string[],
  lang?: string
): Promise<DictionaryTranslation> {
  try {
    const response = await getDictionaryTranslations(lang);
    const result: DictionaryTranslation = {};

    keys.forEach((key) => {
      result[key] = response.translations[key] || key;
    });

    return result;
  } catch (error) {
    console.warn(`Failed to fetch dictionary translations for keys:`, error);
    // Return keys as fallback values
...
/**
 * Get common navigation translations
 * @param lang - Language code (e.g., 'en', 'fi'). If not provided, uses current language
 * @returns Promise<DictionaryTranslation>
 */
export async function getNavigationTranslations(
  ...
  return getDictionaryTranslationsByKeys(navigationKeys, lang);
}

/**
 * Get common button translations
 * @param lang - Language code (e.g., 'en', 'fi'). If not provided, uses current language
 * @returns Promise<DictionaryTranslation>
 */
export async function getButtonTranslations(
  lang?: string
): Promise<DictionaryTranslation> {
  const buttonKeys = [
    "btn_submit",
  ...
  return getDictionaryTranslationsByKeys(buttonKeys, lang);
}

/**
 * Get common form translations
 * @param lang - Language code (e.g., 'en', 'fi'). If not provided, uses current language
 * @returns Promise<DictionaryTranslation>
 */
export async function getFormTranslations(
  ...
  return getDictionaryTranslationsByKeys(formKeys, lang);
}
